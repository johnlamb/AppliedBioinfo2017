...
def main():
    ...
    parser.add_option("-s", "--min-separation", type=int, default=7, help="Set\
                the minimum sequence separation of pairs to be outputted\
                [default: %default]")
    parser.add_option("-n", "--num-contacts", type=int, default=30, help="Set\
            the number of pairs to output [default: %default]")
...
def get_top_pairs(mat, num_contacts, min_separation):
    ...
    top = ((top % mat.shape[0]).astype(np.uint16),
           np.floor(top / mat.shape[0]).astype(np.uint16))
    return top
